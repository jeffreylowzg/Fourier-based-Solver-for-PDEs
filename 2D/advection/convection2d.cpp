...
//   u_t + v·grad(u) = D ∇²u
// with constant velocity (vx,vy) and diffusion coefficient D.
// Here, we assume the solution is defined on [0,L]×[0,L] but we
// want to enforce nonperiodic boundaries (e.g. Dirichlet zero),
// so after the inverse FFT we set the boundary values to zero.
//-------------------------------------------------------------------
                                     ...
                                     double L)
{
    ...
    // Copy real-space data into FFTW input array.
    for (int i = 0; i < nc; i++) {
        for (int j = 0; j < nc; j++) {
            in[i * nc + j] = u(i, j);
        }
    ...
    for (int i = 0; i < nc; i++) {
        ...
        for (int j = 0; j < nc/2 + 1; j++) {
            ...
            // Compute lambda = -D*k^2 - i*(kx*vx+ky*vy)
            double lambda_r = -D * ksq;
            double lambda_i = -(kx * vx + ky * vy);

            int index = i * (nc/2 + 1) + j;
            double a = out[index][0]; // real part
            double b = out[index][1]; // imaginary part

            // For zero mode, no update is necessary.
            if (ksq == 0.0 && lambda_i == 0.0)
                continue;

            // RK4 stages (complex arithmetic):
...
// Using the formula:
//    U^(n+1) = U^(n) / (1 - dt * λ),
// with λ = -D*k^2 - i*(kx*vx+ky*vy).
// After the inverse FFT, we enforce Dirichlet boundaries.
//-------------------------------------------------------------------
                                    ...
                                    double L)
{
    ...
    // Apply Backward Euler in Fourier space.
    for (int i = 0; i < nc; i++) {
        int k_i = (i <= nc/2) ? i : i - nc;
        double kx = 2.0 * M_PI * k_i / L;
        for (int j = 0; j < nc/2 + 1; j++) {
            ...
            // Denom = 1 - dt*(lambda_r + i*lambda_i)
